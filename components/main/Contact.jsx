...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setMessages([
      ...messages,
      { ...formData, id: messages.length + 1 }, // پیام جدید اضافه می‌شود
    ]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-6xl">
        ...
        {/* کرول جدید برای نمایش پیام‌های ارسال‌شده */}
        {messages.length > 0 && (
            ...
            <Carousel className="w-full md:w-3/5 max-w-4xl mb-4">
              {messages.map((message) => (
                <Carousel.Item key={message.id}>
                  <Card className={`${bgColorContact} text-center p-6 rounded-lg shadow-lg`}>
                    <Card.Body className={`${isDarkMode ? "bg-black" : "bg-red-100"}`}>
                      <h4 className={`text-xl font-semibold ${isDarkMode ? "text-yellow-500" : "text-yellow-950"}`}>
                        پیام از {message.name}
                      </h4>
                      <p className={`text-lg ${isDarkMode ? "text-yellow-500" : "text-yellow-950"}`}>
                        {message.message}
                      </p>
                      <p className={`text-md ${isDarkMode ? "text-yellow-500" : "text-yellow-950"}`}>
